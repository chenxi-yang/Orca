...
from rl_module.util.replay_buffer import BootstrapIterator, TransitionIterator
...
class ModelTrainer:
    ...
    def train(
        ...
        best_val_score, infity_score = self.evaluate(eval_dataset) if evaluate else None
        ...
        for epoch in epoch_iter:
            ...
            model_val_score = 0
            if evaluate:
                eval_score, eval_infty_score = self.evaluate(
                    eval_dataset, batch_callback=batch_callback_epoch
                )
                val_scores.append(eval_score.mean().item())
                # print(eval_infty_score)
                # l_infty_scores.append(eval_infty_score.tolist())

                l_infty_scores.append(eval_infty_score.mean().item())
        ...
        return training_losses, val_scores, l_infty_scores
    ...
    def evaluate(
        ...
        batch_scores_list = []
        batch_infty_scores_list = []
        for batch in dataset:
            batch_score, batch_infty_score, meta = self.model.eval_score(batch)
            batch_scores_list.append(batch_score)
            batch_infty_scores_list.append(batch_infty_score)
            if batch_callback:
        ...
        try:
            ...
            batch_infty_scores = torch.cat(
                batch_infty_scores_list, dim=batch_infty_scores_list[0].ndim - 2
            )
        ...
        mean_axis = 1 if batch_infty_scores.ndim == 2 else 2
        #TODO to use mean or max TODO
        # print(batch_infty_scores.shape)
        # batch_infty_scores = batch_infty_scores.mean(dim=mean_axis)
        # batch_infty_scores = torch.amax(batch_infty_scores, dim=mean_axis)
        batch_infty_scores = torch.mean(batch_infty_scores, dim=mean_axis)
        # print(f"shape after mean: {batch_infty_scores.shape}")
        # exit(0)

        return batch_scores, batch_infty_scores

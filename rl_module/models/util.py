...
def truncated_normal_init(m: nn.Module):
    ...
    if isinstance(m, nn.Linear) or isinstance(m, nn.Linear):
        ...
        rl_module.util.math.truncated_normal_(m.weight.data, std=stddev)
        m.bias.data.fill_(0.0)
    if isinstance(m, EnsembleLinearLayer):
        num_members, input_dim, _ = m.weight.data.shape
        stddev = 1 / (2 * np.sqrt(input_dim))
        for i in range(num_members):
            rl_module.util.math.truncated_normal_(m.weight.data[i], std=stddev)
        m.bias.data.fill_(0.0)
...
def to_tensor(x):
    ...
    if isinstance(x, vrl.domain.box.Box):
        return x.to_tensor()

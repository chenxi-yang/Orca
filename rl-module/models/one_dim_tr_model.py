...
import vrl.models.util as model_util
...
class OneDTransitionRewardModel(Model):
    ...
    To use with :class:vrl.models.ModelEnv`, the wrapped model must define methods
    ``reset_1d`` and ``sample_1d``.

    Args:
        model (:class:`vrl.model.Model`): the model to wrap.
    ...
    def __init__(
        ...
        obs_process_fn: Optional[vrl.util.types.ObsProcessFnType] = None,
        ...
        self.model = model
        self.input_normalizer: Optional[vrl.util.math.Normalizer] = None
        if normalize: # true in core
            self.input_normalizer = vrl.util.math.Normalizer(
                self.model.in_size,
                self.model.device,
    ...
    def _get_model_input(
        self,
        obs: vrl.util.types.TensorType,
        action: vrl.util.types.TensorType,
    ) -> torch.Tensor:
    ...
    def _process_batch(
        self, batch: vrl.util.types.TransitionBatch, _as_float: bool = False
    ...
    def update_normalizer(self, batch: vrl.util.types.TransitionBatch):
        ...
        Args:
            batch (:class:`vrl.util.types.TransitionBatch`): The batch of transition data.
    ...
    def loss(
        self,
        batch: vrl.util.types.TransitionBatch,
    ...
    def get_output_and_targets(
        self, batch: vrl.util.types.TransitionBatch

...
def main():
    ...
    if is_learner: # is learner
        ...
        while env_steps < config.learner_num_steps: # params.dict['max_epochs']: # 1m epochs
            ...
            freq_train_model = 64 # every epoch, train the model once (64 is also the evaluation step)
            epoch_length = 64 # length of one-time TCP interaction

            num_ac_updates_per_step = 1 # number of updates to the learner
...
def core():
    ...
    if is_learner:
        ...
        while epochs < config.learner_epochs:
            ...
            for data in buffer_data:
                agent.store_model_experience(data[0], data[1], data[2], data[3], data[4])
